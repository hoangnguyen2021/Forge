...
class Camera2Session(private val context: Context) {
    ...
    // Handler tied to cameraThread's Looper — passed to Camera2 so it knows which thread
    // to post its callbacks onto.
    ...
    private fun startPreview(camera: CameraDevice, surface: Surface) {
        // createCaptureSession is also async — onConfigured fires when the pipeline is ready.
        // We pass our Surface as the only output target: all frames go to this Surface.
